...
sequenceDiagram
    ...
    participant Gemini as Gemini API (動的モデル解決)
    ...
    GAS->>Gemini: 利用可能な最新モデル一覧を取得 (models.list)
    Gemini-->>GAS: 利用可能モデルリスト返却
    
    loop 各新着記事
        GAS->>WebPage: 記事本文をスクレイピング
        WebPage-->>GAS: 本文テキスト (HTMLをクレンジング)
        GAS->>Gemini: 本文テキストを渡して要約を依頼 (最適モデルで試行・自動フォールバック)
        Gemini-->>GAS: 日本語の要約
...
2. **堅牢性とエラーハンドリング**:
   - **実行時間・API制限への配慮と安全終了ガード**: GASの実行時間制限（最大6分）およびGemini APIの無料枠制限（15 RPM）を考慮し、1回あたりの最大処理件数を **5件** に制限。また、処理開始から **4分（240秒）** を超過した場合は残りの記事の処理を安全に切り上げて正常終了するタイムアウト監視ガードを搭載。さらにAPIリクエスト間には **1.5秒のウェイト** を挿入。
   - **例外処理の徹底**: 各記事の処理を独立した `try-catch-finally` で囲み、一部の記事でスクレイピングや要約が失敗しても、残りの記事の処理に影響を与えない堅牢な設計。
   - **アクセス不可・短文記事のスキップ・記録**: 記事本文が403/404エラー等で取得できない場合や、抽出された本文テキストが極端に短い場合（**150文字未満**。Cloudflareによるボット防御画面や、SPAサイトの空のスケルトンHTMLの可能性があるため）は要約をスキップし、Slackに警告（⚠️）とスキップ理由を通知。スキップされた記事も送信済履歴（スプレッドシート）に追加されるため、二重に処理されることはありません。
   - **Gemini APIの動的モデル解決と自動フォールバック・早期サーキットブレーカー**: 処理開始時に `models.list` APIから利用可能なモデル一覧を動的に取得し、テキスト生成に対応したFlash系モデル（`gemini-2.5-flash`, `gemini-2.5-flash-lite`, `gemini-3.5-flash` など）を優先順に上位 **最大3件** に絞り込んで自動選定します。これにより旧世代モデルの廃止（シャットダウン）による404エラー連鎖を防ぎ、新モデル登場時もコード変更なしで自動追従します。一時的な混雑（HTTP 503）時は指数バックオフで再試行し、モデル未対応等（HTTP 404）時は次候補モデルへ自動フォールバックします。無料枠のレートリミット（HTTP 429）や不正リクエスト（HTTP 400）を検知した場合は即座にサーキットブレーカー（Gemini要約スキップ）が作動し、無駄なリトライ探索によるGASタイムアウトを防止します。安全フィルター（SAFETY等）でブロックされた場合はエラー理由（`理由: SAFETY` など）を要約欄に表示します。
3. **Slack上での情報完結**:
...
- **検証範囲**: [src/Code.test.js](src/Code.test.js) にて、トークン認証、エラーハンドリング、二重送信防止、Gemini APIの動的モデル取得とフィルタリング・最大件数制限、一時的な API 障害時のリトライ/リカバリ処理、レートリミット（429）時の即時サーキットブレーカー、アクセス不可記事・短文コンテンツのスキップ仕様、Gemini APIの安全性フィルターブロック時のハンドリング、および実行時間上限ガード機能など、全33件のテストケースを網羅しています。
