...
2. **堅牢性とエラーハンドリング**:
   ...
   - **Gemini APIのエラーハンドリング強化と自動モデルフォールバック**: 安全フィルター（SAFETY等）や引用フィルター（RECITATION等）で要約生成がブロックされた場合、余計なリトライを行わずにエラー理由（`理由: SAFETY` など）を要約欄に表示します。また、優先モデル（`gemini-3.5-flash`）が混雑による一時的エラー（503/429）やモデル未対応・無効エラー（404/400）を返した場合、自動的に安定した代替モデル（`gemini-2.5-flash` ➔ `gemini-2.0-flash` ➔ `gemini-1.5-flash`）を順に試行する**自動フォールバック機能**が作動します。すべてのモデル候補で失敗した場合は**サーキットブレーカー（一時利用停止）**が作動し、同一実行回内の以降の記事に対する無駄なAPI呼び出しを抑止（スキップ）します。
3. **Slack上での情報完結**:
...
- **検証範囲**: [src/Code.test.js](src/Code.test.js) にて、トークン認証、エラーハンドリング、二重送信防止、一時的な API 障害時のリトライ/リカバリ処理、アクセス不可記事・短文コンテンツのスキップ仕様、Gemini APIの安全性フィルターブロック時のハンドリング、およびサーキットブレーカー機能など、全26件のテストケースを網羅しています。
