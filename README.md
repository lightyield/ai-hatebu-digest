...
2. **堅牢性とエラーハンドリング**:
   ...
   - **Gemini APIの動的モデル解決と自動フォールバック**: 処理開始時に `models.list` APIから利用可能なモデル一覧を動的に取得し、テキスト生成に対応したFlash系モデル（`gemini-2.5-flash`, `gemini-2.5-flash-lite`, `gemini-3.5-flash` など）を優先順に自動選定します。これにより、旧世代モデルの廃止（シャットダウン）による404エラー連鎖を防ぎ、新モデル登場時もコード変更なしで自動追従します。また、一時的な混雑（HTTP 503/429）時は指数バックオフで再試行し、モデル未対応等のエラー時は次候補モデルへ自動フォールバックします。安全フィルター（SAFETY等）でブロックされた場合はエラー理由（`理由: SAFETY` など）を要約欄に表示し、すべての候補で失敗した場合のみサーキットブレーカー（一時利用停止）が作動します。
3. **Slack上での情報完結**:
...
- **検証範囲**: [src/Code.test.js](src/Code.test.js) にて、トークン認証、エラーハンドリング、二重送信防止、Gemini APIの動的モデル取得とフィルタリング・ソート、一時的な API 障害時のリトライ/リカバリ処理、アクセス不可記事・短文コンテンツのスキップ仕様、Gemini APIの安全性フィルターブロック時のハンドリング、およびサーキットブレーカー機能など、全31件のテストケースを網羅しています。
