...
2. **堅牢性とエラーハンドリング**:
   ...
   - **Gemini APIのエラーハンドリング強化**: 安全フィルター（SAFETY等）や引用フィルター（RECITATION等）で要約生成がブロックされた場合、余計なリトライを行わずにエラー理由（`理由: SAFETY` など）を要約欄に表示します。また、APIキー無効などの恒常的エラー（400等）や制限超過（429等）でリトライが全て失敗した場合は**サーキットブレーカー（一時利用停止）**が作動し、同一実行回内の以降の記事に対する無駄なAPI呼び出しを抑止（スキップ）します。
3. **Slack上での情報完結**:
